...
4.5 **実機性能の検証優先事項**:
   - まずは実機でもシミュレーションと同じ入力形式・周波数で動作することを確認する。
   - 実機ではセンサーのノイズ、遮蔽、反射特性が異なるため、同じモデルでも挙動が変わる可能性がある。
   - 推論遅延が制御周期に収まるかを必ず測定し、40Hz が現実的に維持できない場合は制御周期を落とす。
   - 実機では「平均速度が高くても完走率が低い」モデルは危険であり、安定性を優先してから速度を上げる。
   - 実機評価の前に、`scripts/evaluate.py` によるシミュレーション評価結果を参考にしつつ、必ず現地テストを行う。
