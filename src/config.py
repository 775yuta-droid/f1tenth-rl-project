...
MIN_SPEED = float(os.environ.get("MIN_SPEED", "0.3"))   # 低速停止旋回を抑制しつつ、短時間での前進学習を促す
MAX_SPEED = float(os.environ.get("MAX_SPEED", "0.7"))   # [Fix-A2] 1.0 → 2.0: 報酬関数の max_speed と整合し、カーブ報酬を正常化
...
START_POSES = [
    [0.39, -0.61, 0.15],  # Pose 0
    [3.03, -1.44, -0.46],  # Pose 1
    [2.38, -5.01, -2.55],  # Pose 2
    [2.12, -1.07, -0.45],  # Pose 3
    [-0.73, -5.60, -1.73],  # Pose 4
    [-2.14, -10.20, 1.46],  # Pose 5
    [-1.21, -11.23, 3.08],  # Pose 6
    [-2.03, -7.70, 1.82],  # Pose 7
    [3.13, -3.36, -1.95],  # Pose 8
    [-0.15, -7.45, -1.21],  # Pose 9
